...
public interface NamingConversition {
  ...
  /**
   * 工厂方法，获取默认的命名转换实现（不进行任何转换）。
   */
  static NamingConversition ofNop() {
    return Nop.INST;
  }
   ...
   * 工厂方法，获取下划线命名转换实现。
   */
  static NamingConversition ofSnake() {
    return new SnakeNamingConversition();
  }

  /**
   * 工厂方法，获取驼峰命名转换实现。
   */
  static NamingConversition ofCamel() {
    return new CamelNamingConversition();
  }
